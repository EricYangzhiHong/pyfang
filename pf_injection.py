...
def html_diff(before_injection, after_injection):
...
def get_num_columns(page):
    count = 1

    original_page = get_page(page)
    union = page + '%20UNION%20SELECT%201'

    # Heuristic just looks for SELECT, needs to be MORE NUANCED!
    while 'SELECT' in html_diff(original_page, get_page(union)):
        count += 1
        union += ',%20' + str(count)
...
def injection(page, params):
    ...
    for param in params:
        ...
        for i in xrange(0, len(union_urls) + 1):
            ...
            #print new_url
            data[param].append(html_diff(page1, get_page(new_url)))
